// donna-lazy-path.test.mjs - the Donna mission's lazy-route browser gates: the navigation race,
// the corrupt full payload, and the corrupt preview slice, on DONNA'S routes and modules.
//
//   node demo/js/robots/gen-fixture/donna-lazy-path.test.mjs
//
// The ssl-* browser suites exercise `#/demo/ssl` and `robots/ssl/*` by name, so a green run of
// them proves nothing about the page's THIRD lazy mission: a Donna-only race or a corrupt recorded
// module could strand the route while every ssl gate stays green. This suite drives the same three
// contracts through Donna's own route and modules. The contracts themselves live in app.js and
// data.js and are shared; what is Donna-specific here is only the route, the module paths, the
...
const H = harness('donna-lazy-path');
...
/** A page whose ONLY throttled request is Donna's recorded module. */
async function openThrottled() {
  ...
  let arrivals = 0;
  await page.route('**/robots/donna/donna-data.js', async (route) => {
    arrivals++;
...
// The def-owned loading copy, asserted verbatim: this line is Donna's and no other mission's.
const onLoadingCard = (page) =>
  waitFor(
    page,
    () => {
      const m = document.getElementById('ingest-mount');
      return !!m && /Loading the recorded mission/i.test(m.textContent || '');
    },
    8000,
    'the loading card',
  );
...
H.section('A: demo -> picker -> the same demo, inside one load');
{
  ...
  H.ok(st.dataset === 'demo' && st.hash === '#/demo/donna', 'the router and the hash agree');
  H.ok(errors.length === 0, `no uncaught page errors (${errors.slice(0, 2).join(' | ')})`);
  H.ok(hits() >= 1, `the recorded module was requested (${hits()}x)`);
  await ctx.close();
}
...
// ------------------------------------------------------------- D. corrupt recorded module

H.section('D: a corrupt recorded module lands on the unavailable card');
{
  const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const page = await ctx.newPage();
  const errors = [];
  ...
  await page.route('**/robots/donna/donna-data.js', (route) =>
    route.fulfill({
...
H.section('E: a corrupt preview slice degrades the picker card');
{
  ...
  H.ok(st.card, 'the Donna card is on the picker');
